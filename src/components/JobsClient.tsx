...
export function JobsClient({ initialJobs, initialFilters, initialHasMore }: JobsClientProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 py-10 sm:py-14">
      <div className="mb-8">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-orange-600">Job board</p>
        <h1 className="mt-2 text-3xl sm:text-5xl font-bold text-[#050316] font-subheading">Find your next opportunity</h1>
      ...
      <div className="mt-8 flex items-center justify-between border-b border-slate-200 pb-3 text-sm text-slate-500">
        ...
        <button onClick={() => router.refresh()} className="inline-flex items-center gap-1.5 text-xs font-semibold hover:text-orange-600" aria-label="Refresh jobs">
      ...
      {isPending ? (
        ...
        <div className="mt-5 py-16 text-center bg-white border border-slate-200 rounded-xl">
      ...
      {hasMore && (
        <button onClick={loadMore} disabled={loadingMore} className="mt-8 mx-auto flex items-center justify-center gap-2 min-h-11 px-5 rounded-lg bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600 disabled:opacity-60">
